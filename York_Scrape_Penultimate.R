...
ffd <- rd$client
ffd$navigate("https://www.ratemyprofessors.com/search/teachers?query=*&sid=1495") #Currently entered YorkU, CAD, 96 + 1  departments
...
num_check <- page %>% html_nodes("h1") %>% html_text()
num_check <- trunc((strtoi(str_split(num_check, " ")[[1]][1])-8)/8)+2
...
for (i in 1:num_check)
{
  ...
  print(paste("i is ", i, ", ", num_check - i, " more to go"))
  Sys.sleep(8)
}
...
write.csv(profess, "C:\\Users\\Rajat\\Desktop\\Runny\\R\\RateMyProf-Scrape-R\\YorkUPenultimate5.csv") #change uni name each time
...
for (i in 1:nrow(profess)) { # Add skipper for 0 ratings

  check_num_rate <- strtoi(profess[i,5])
  if (check_num_rate > 0){
    ...
    NumRate <- 0
    NumRate <- strtoi(profess[i,5])
    
    if (NumRate >= 20){
      NumRate <- ((NumRate-20)/10) + 2 #number of iterations +2 just to keep it safe
    }
    else{
      NumRate <- NumRate
    }
    ...
    for (a in 1:NumRate)
    {
      suppressMessages(try(load_btn <- ffd$findElement(using = "css", value = ".gjQZal"), silent = TRUE))
      suppressMessages(try(load_btn$clickElement(), silent = TRUE))
      print(a)
      Sys.sleep(2)
    }
    ...
    freshcoursedate$course_name <- substr(freshcoursedate$course_name,1,5)
    ...
    ProfnameSearch <- paste(profess[i,1], profess[i,2], "at", profess[i,8],"RateMyProfessor", sep = " ")
    CSVname <- paste(ProfnameSearch,".csv",sep = "")
  
    DescriptionName <- paste("Description ", ProfnameSearch,".csv",sep = "")
  
    write.csv(description_indv, paste('C:\\Users\\Rajat\\Desktop\\Runny\\R\\RateMyProf-Scrape-R\\YorkU_Professors\\Descriptions\\', DescriptionName, sep = ""), row.names = FALSE) 
  
  #Enter your destination where all description for each professor needs to be
  
    write.csv(professInfo, paste('C:\\Users\\Rajat\\Desktop\\Runny\\R\\RateMyProf-Scrape-R\\YorkU_Professors\\',CSVname, sep = ""), row.names = FALSE) 
